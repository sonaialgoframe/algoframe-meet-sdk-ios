...
#import "AlgoframeMeetConferenceOptions.h"
#import "AlgoframeMeetViewDelegate.h"
...
@interface AlgoframeMeetView : UIView

@property (nonatomic, nullable, weak) id<AlgoframeMeetViewDelegate> delegate;

/**
 * Joins the conference specified by the given options. The given options will
 * be merged with the defaultConferenceOptions (if set) in AlgoframeMeet. If there
 * is an already active conference it will be automatically left prior to
 * joining the new one.
 */
- (void)join:(AlgoframeMeetConferenceOptions *_Nullable)options;
/**
