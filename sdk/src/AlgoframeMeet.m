...
#import "AlgoframeMeet+Private.h"
#import "AlgoframeMeetConferenceOptions+Private.h"
#import "AlgoframeMeetView+Private.h"
...
#if !defined(ALGOFRAME_MEET_SDK_LITE)
#import <RNGoogleSignin/RNGoogleSignin.h>
#import "Dropbox.h"
#endif

@implementation AlgoframeMeet {
...
+ (instancetype)sharedInstance {
    static AlgoframeMeet *sharedInstance = nil;
...
-    (BOOL)application:(UIApplication *)application
  continueUserActivity:(NSUserActivity *)userActivity
    ...
    AlgoframeMeetConferenceOptions *options = [self optionsFromUserActivity:userActivity];
    if (options) {
        [AlgoframeMeetView updateProps:[options asProps]];
        return true;
    }
...
- (BOOL)application:(UIApplication *)app
    ...
    AlgoframeMeetConferenceOptions *conferenceOptions = [AlgoframeMeetConferenceOptions fromBuilder:^(AlgoframeMeetConferenceOptionsBuilder *builder) {
        builder.room = [url absoluteString];
    }];
    [AlgoframeMeetView updateProps:[conferenceOptions asProps]];
...
- (AlgoframeMeetConferenceOptions *)getInitialConferenceOptions {
    if (_launchOptions[UIApplicationLaunchOptionsURLKey]) {
        NSURL *url = _launchOptions[UIApplicationLaunchOptionsURLKey];
        return [AlgoframeMeetConferenceOptions fromBuilder:^(AlgoframeMeetConferenceOptionsBuilder *builder) {
            builder.room = [url absoluteString];
        }];
    } else {
...
- (BOOL)isCrashReportingDisabled {
    NSUserDefaults *userDefaults = [[NSUserDefaults alloc] initWithSuiteName:@"algoframe-default-preferences"];
    return [userDefaults stringForKey:@"isCrashReportingDisabled"];
}

- (AlgoframeMeetConferenceOptions *)optionsFromUserActivity:(NSUserActivity *)userActivity {
...
- (void)setDefaultConferenceOptions:(AlgoframeMeetConferenceOptions *)defaultConferenceOptions {
