...
@implementation AlgoframeMeetView {
    /**
...
- (void)join:(AlgoframeMeetConferenceOptions *)options {
...
- (void)hangUp {
    ExternalAPI *externalAPI = [[AlgoframeMeet sharedInstance] getExternalAPI];
 ...
 *
 * - AlgoframeMeet.defaultConferenceOptions
 * - This function's parameters
 * - Some extras which are added by this function
 */
- (void)setProps:(NSDictionary *_Nonnull)newProps {
    NSMutableDictionary *props = mergeProps([[AlgoframeMeet sharedInstance] getDefaultProps], newProps);
    ...
    if (rootView) {
        ...
        RCTBridge *bridge = [[AlgoframeMeet sharedInstance] getReactBridge];
        rootView
