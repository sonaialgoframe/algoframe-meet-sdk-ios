...
class InCallController: WKInterfaceController {
    ...
    @IBAction func hangupClicked() {
      sendCommand(AlgoframeMeetCommands.CMD_HANG_UP, message: nil)
    }

    @IBAction func muteClicked() {
        if var micMuted = ExtensionDelegate.currentAlgoframeMeetContext.micMuted {
            micMuted = !micMuted;
            sendCommand(
                AlgoframeMeetCommands.CMD_SET_MUTED,
                message: [
  ...
  func sendCommand(_ command: AlgoframeMeetCommands, message: [String : Any]?) {
        if WCSession.isSupported() {
            let session = WCSession.default
            var data = [String: Any]()

            if let sessionID = ExtensionDelegate.currentAlgoframeMeetContext.sessionID {
                if message != nil {
    ...
    func updateUI(_ newContext: AlgoframeMeetContext) {
        var conferenceURL = newContext.conferenceURL

        if let joinConferenceURL = newContext.joinConferenceURL {
            sendCommand(AlgoframeMeetCommands.CMD_JOIN_CONFERENCE, message: [ "data" : joinConferenceURL ])
    ...
    override func awake(withContext context: Any?) {
        ...
        if let data = context as? AlgoframeMeetContext {
          updateUI(data)
        }
    }
}
