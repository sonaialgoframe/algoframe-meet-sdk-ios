...
class InterfaceController: WKInterfaceController {
    ...
    override func didAppear(){
        self.updateUI(ExtensionDelegate.currentAlgoframeMeetContext)
    }
    ...
    private func updateRecents(withRecents recents: NSArray, currentContext: AlgoframeMeetContext) {
    ...
    override func contextForSegue(withIdentifier segueIdentifier: String, in table: WKInterfaceTable, rowIndex: Int) -> Any? {
        ...
        let currentContext = ExtensionDelegate.currentAlgoframeMeetContext

        // Copy the current context and add the joinConferenceURL to trigger the command when the in-call screen is displayed
        let actionContext = AlgoframeMeetContext(jmContext: currentContext)
